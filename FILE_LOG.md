...
| 12 | 2026-07-31 ~09:12 | Restored (source on `main`) | Full public site tree → pre-V3 remodel tip | `main` @ `3171b4c` tree-identical to tag `pre-v3-remodel-2026-07-19` / `1496da8`; pushed → Netlify production | Owner: MS Clarity Google-referral sessions showed guest confusion / exit without booking on V3 IA. Non-destructive history (new commit, no force-push). Branch `restore/pre-v3-remodel` also pushed. V3 tip still reachable at `597b43a`. Local WIP stashed as `wip-before-pre-v3-restore-2026-07-31` + `wip2-pre-v3-restore`. |
| 13 | 2026-07-31 ~09:35 | Restored + edited | `public/images/northern-lights-chris-olson.jpg` + Supabase `pages` index Interlude `backgroundImageUrl` | Image restored from `597b43a` → `main` @ `ff557b3`; DB Interlude pointed at `/images/northern-lights-chris-olson.jpg` (was empty → navy gradient). One-shot script archived → `_Archive\one-shot-scripts-2026-07-31\` | Dark Skies home section missing guest aurora/nebula photo after pre-V3 rollback. |
| 14 | 2026-07-31 ~09:46 | Restored + edited | Google Place photo proxy + Area Guide wiring | `place-photo.ts` API + `lib/place-photo.ts` from `597b43a`; LocalPlaceCard / `getRegionMapPins` / things+trails heroes prefer `?place=`; CSP + `.env.example` Maps/Places allowlist restored | Pre-V3 restore had dropped Place-ID refresh + Maps CSP; Area Guide images/map thumbs failed. |
| 15 | 2026-07-31 ~10:00 | Archived | Guest Check Availability page | `src/pages/availability.astro`, `src/pages/api/availability.ts`, `src/components/AvailabilityMap.astro` → `_Archive\retired-availability-page-2026-07-31\`; nav link removed; `/availability` 302 → Firefly | Owner: cannot keep map current; page creates guest friction. |
